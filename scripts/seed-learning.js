...
async function seedLearningData() {
  try {
    ...
    const cchAxcess = await LearningApplication.create({
      name: 'CCH Axcess',
    ...
    console.log('Created CCH Axcess application');

    // Module 1: Introduction
    const module1 = await LearningModule.create({
      applicationId: cchAxcess._id,
...
- Preparers: Prepare and review tax returns
- Reviewers: Review and approve returns
- Partners: Final approval and e-filing`,
        importantNotes: [
...
- Client Manager: Manage client information and relationships
- Return Manager: Access and manage tax returns
- Documents: Access document management system
- Tools: Additional utilities and settings
- Help: Access support resources

Search Functionality:
- Use the search bar at the top to find clients, returns, or documents
- Search by client name, SSN, or return ID
- Filters available for date ranges and return types`,
        importantNotes: [
...
4. Manager/Partner Review
   - Reviewer examines the return
   - Verifies calculations and compliance
   - Checks for tax planning opportunities
   - Provides feedback or approves

5. Client Review
   - Return is shared with client for review
   - Client provides approval or changes
   - Final adjustments made if needed

6. E-filing
   - Return is electronically filed with IRS/state
   - Tracking of filing status
   - Acknowledgment receipt confirmation

7. Archiving
   - Return is archived for record retention
   - Documents stored in client folder
   - Return available for future reference`,
        importantNotes: [
...
4. Save changes
5. System logs all changes for audit trail

Managing Client Documents:
- Upload documents to client folder
- Organize by tax year or document type
- Set document permissions
- Share documents securely with clients

Client Communication:
- Send messages through portal
...
- Diagnostics automatically check for errors
- Review all diagnostic messages
- Address critical errors first
- Document resolutions for audit trail

Printing and Exporting:
- Print return for client review
- Export to PDF for electronic sharing
- Print government filing copies
- Generate client letters and summaries`,
        importantNotes: [
...
Document Upload:
1. Go to client document folder
2. Click "Upload Document"
3. Select file from computer
4. Categorize document type
5. Add description if needed
6. Set appropriate permissions

Data Verification:
- Always verify imported data against source documents
...
Best Practices:
- Run diagnostics frequently during preparation
- Address critical errors immediately
- Review warnings even if not required
- Keep diagnostic history for audit trail`,
        importantNotes: [
    ...
    console.log(`Created ${await LearningApplication.countDocuments()} applications`);
    console.log(`Created ${await LearningModule.countDocuments()} modules`);
    console.log(`Created ${await LearningLesson.countDocuments()} lessons`);
