...
async function addQuickBooksAcademyModule() {
  try {
    ...
    if (!academy) {
      console.log('Academy application not found');
      return;
    }
    console.log('Found Academy application:', academy._id);

    // Check if module already exists
    const existingModule = await LearningModule.findOne({
      applicationId: academy._id,
      name: 'QuickBooks Comprehensive Operations & Support'
    });

    if (existingModule) {
      console.log('QuickBooks Academy module already exists. Updating lesson...');
      const moduleId = existingModule._id;
      
      // Update order to 12
      await LearningModule.findByIdAndUpdate(moduleId, { order: 12 });
      console.log('Updated module order to 12');
      ...
      // Create new module with order 12
      const newModule = await LearningModule.create({
        applicationId: academy._id,
        name: 'QuickBooks Comprehensive Operations & Support',
        description: 'Master QuickBooks from setup to production support including internal architecture, business workflows, and troubleshooting',
        order: 12,
        isActive: true,
      });
      console.log('Created new module with order 12:', newModule._id);
      
      await createComprehensiveLesson(newModule._id);
    }

    console.log('✅ QuickBooks Academy module added successfully');
